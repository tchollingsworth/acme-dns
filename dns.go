...
func (d *DNSServer) readQuery(m *dns.Msg, remote net.Addr) {
	var authoritative = false
	for _, que := range m.Question {
		if rr, rc, auth, err := d.answer(que, remote); err == nil {
			if auth {
				authoritative = auth
			}
			m.MsgHdr.Rcode = rc
...
func (d *DNSServer) answer(q dns.Question, remote net.Addr) ([]dns.RR, int, bool, error) {
	var rcode int
	var err error
	var txtRRs []dns.RR
